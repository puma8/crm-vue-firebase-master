  ...
  <q-scroll-area class="full-height">
    ...
    <div v-if="hasCustomFields" class="row q-gutter-y-md q-pa-md">
      <div
        ...
        v-for="detail in project.customFields"
        :key="detail.id"
      >
        <div class="col-11 row">
          <div class="col-5">{{customFieldNameById(detail.id)}}</div>
          <custom-field-value class="col-7" :custom-field="detail" entity='project' />
        </div>
        <div class="col-1">
    ...
    <div class="primary-bg-color q-pa-md project-main__add-detail" v-if="$permissionResolver.hasPermission('create', 'projectCustomField')">
      <div class="text-weight-medium q-pb-md">Details</div>
      <div>What else do you know about the project?</div>
      <q-btn color="primary" flat dense size="sm" label="+Add details" @click="addDetails" />
    </div>
    <div class="row q-gutter-y-md q-pa-md">
      <div class="col-12" v-for="(contactGroup, index) in projectContacts" :key="index">
        <div class="q-mb-xs">
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      teamLeadById: 'projects/teamLeadById',
      people: 'people/list',
      users: 'users/list',
    ...
    hasCustomFields () {
      ...
      return (this.project.customFields || []).length > 0
    }
  },
  watch: {
  ...
  methods: {
    setData (project) {
      ...
      this.revenue = project.revenue ? `$${project.revenue}` : 'N/A'
      this.members = project.members || []
      this.newTeamLeadId = this.project.teamLeadId
      this.newPeople = this.project.people || []
    },
    customFieldNameById (id) {
      return (this.customFieldById(id, 'project') || {}).fieldName
    },
    refreshContacts () {
      const projectContacts = _.filter(this.people, ({ id }) => _.indexOf(this.project.people, id) !== -1)
      const enhancedContacts = projectContacts.map(contact => {
    ...
    addDetails () {
      this.$refs.ManageCustomFieldDialog.open(null, async customField => {
        const respopnse = await this.$firebase.setting.project.customField.create(customField)
        const id = respopnse.data
        const customFields = (this.project.customFields || []).concat({ id, fieldValue: customField.fieldValue })
        return this.$firebase.project.update({ ...this.project, customFields })
      }, (_, error) => {
    ...
    editDetail (detail) {
      const customField = {
        ...this.customFieldById(detail.id, 'project'),
      ...
      this.$refs.ManageCustomFieldDialog.open(customField, async (updatedCustomField, actionType) => {
        if (actionType === 'manage') {
          await this.$firebase.setting.project.customField.update(updatedCustomField)

          const { id, fieldValue } = updatedCustomField
          const customFields = this.project.customFields.map(cf => cf.id !== updatedCustomField.id ? cf : { id, fieldValue })

          return this.$firebase.project.update({ ...this.project, customFields })
        } else if (actionType === 'delete') {
          return this.$firebase.setting.project.customField.delete(customField)
        }
      }, (_, error) => {
    ...
    async updateMembers (members) {
      this.updateProject({ ...this.project, members }, 'members')
    },
    updateTeamLead (value) {
      this.updateProject({ ...this.project, teamLeadId: value }, 'teamLeadId')
    },
    updateRevenue (value) {
      this.updateProject({ ...this.project, revenue: value }, 'revenue')
    },
    updatePeople (value) {
      this.updateProject({ ...this.project, people: value }, 'people')
    },
    updateProject (updatedproject, field) {
      this.$set(this.loading, field, true)
      this.$firebase.project.update(updatedproject)
        .then(() => {
